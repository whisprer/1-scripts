...
def triage_regressions(tmp: Path) -> None:
    """'triage' against the sort of mess that's actually on a NAS: repos with no commits,
    a corrupt .git, no origin, a detached HEAD, a linked worktree, unreadable and
    oddly-named folders, system junk. None of it may stop the report."""
    print("\n== triage regressions")
    w = World(tmp)
    gh, P, N, work = w.gh, w.p52, w.nas, w.work
    for name in ("cool-project", "fractal-lab", "synth-tools", "solver"):
        seed(gh, ME, name, work)
        clone_to(ME, name, P / name)

    root = tmp / "share"
    # a tree with current names, flat: the one triage should recommend
    good = root / "GitHub-Repos"
    good.mkdir(parents=True)
    for name in ("cool-project", "fractal-lab", "synth-tools", "solver"):
        clone_to(ME, name, good / name)
    # a July-style tree with old names
    july = root / "GitHub-Repos-2025-07-03"
    july.mkdir()
    clone_to(ME, "cool-project", july / "CoolProject")
    clone_to(ME, "fractal-lab", july / "Fractal_Lab")
    # the hostile tree
    bad = root / "GitHub-Repos-backup-2"
    bad.mkdir()
    run_git(None, "init", "-q", "-b", "main", str(bad / "empty-no-commits"))   # no commits at all
    clone_to(ME, "solver", bad / "no-origin")
    run_git(bad / "no-origin", "remote", "remove", "origin")
    clone_to(ME, "synth-tools", bad / "detached")
    run_git(bad / "detached", "checkout", "-q", "--detach")
    (bad / "corrupt-git" / ".git").mkdir(parents=True)                         # .git with nothing in it
    clone_to(ME, "cool-project", bad / "worktree-host")
    run_git(bad / "worktree-host", "worktree", "add", "-q", str(bad / "linked-wt"), "-b", "wt")
    (bad / "totally-empty").mkdir()
    (bad / "loose-file.txt").write_text("not a repo\n", encoding="utf-8")
    odd = bad / "weird name (copy) #2"
    odd.mkdir()
    clone_to(ME, "fractal-lab", odd / "fractal-lab")
    ...
    if os.name != "nt":
        ...
        locked = True
    # repos under an owner folder, and one buried deeper than --depth
    (root / "GitHub-Repos-owner" / "whisprer").mkdir(parents=True)
    clone_to(ME, "solver", root / "GitHub-Repos-owner" / "whisprer" / "solver")
    (root / "burrowed" / "a" / "b" / "c").mkdir(parents=True)
    clone_to(ME, "solver", root / "burrowed" / "a" / "b" / "c" / "buried")
    clone_to(ME, "synth-tools", root / "stray-copy")                           # loose at the top
    (root / "#recycle" / "junk").mkdir(parents=True)
    (root / "System Volume Information").mkdir()

    logf = tmp / "triage.txt"
    cmd = [sys.executable, str(SCRIPT), "triage", "--root", str(root), "--p52-base", str(P),
           "--nas-base", str(N), "--size", "--log", str(logf)]
    pr = subprocess.run(cmd, env=ENV, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding="utf-8")
    if locked:
        os.chmod(unreadable, 0o700)
    o = pr.stdout
    check(pr.returncode == 0 and "Traceback" not in o,
          "triage survives empty/corrupt/origin-less/detached repos and unreadable folders")
    check("empty-no-commits" not in o or "Traceback" not in o,
          "triage: a repo with no commits doesn't stop the report")
    check("GitHub-Repos-backup-2" in o and "GitHub-Repos-2025-07-03" in o and "(loose at the top level)" in o,
          "triage: every place on the share is listed, loose repos included")
    check("#recycle" not in o and "System Volume Information" not in o,
          "triage: system folders are left out")
    check("nested (0/1 flat)" in o, "triage: an owner-bucketed tree is flagged as nested")
    rec = [ln for ln in o.splitlines() if "Keep as the live mirror" in ln]
    check(len(rec) == 1 and str(good) in rec[0], f"triage: recommends the flat, current tree (got {rec})")
    check("burrowed" not in o, "triage: a repo below --depth isn't counted")
    check(logf.is_file() and "Suggested plan" in logf.read_text(encoding="utf-8"),
          "triage: --log saves the whole report")

    deeper = subprocess.run([sys.executable, str(SCRIPT), "triage", "--root", str(root),
                             "--p52-base", str(P), "--nas-base", str(N), "--depth", "5"],
                            env=ENV, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding="utf-8")
    check(deeper.returncode == 0 and "burrowed" in deeper.stdout, "triage: --depth 5 reaches a deeper copy")

    missing = subprocess.run([sys.executable, str(SCRIPT), "triage", "--root", str(tmp / "nope")],
                             env=ENV, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding="utf-8")
    check(missing.returncode != 0 and "not found" in missing.stdout, "triage: a missing --root stops cleanly")

    # and the copies triage points at must survive the dupes pass it recommends
    dup = subprocess.run([sys.executable, str(SCRIPT), "dupes", "--root", str(root), "--p52-base", str(P),
                          "--nas-base", str(good), "--apply", "--yes"],
                         env=ENV, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding="utf-8")
    check(dup.returncode in (0, 1) and "Traceback" not in dup.stdout,
          "dupes survives the same hostile copies")
    check((bad / "worktree-host").exists() and (bad / "linked-wt").exists(),
          "dupes keeps a copy that has another working tree attached to it")
    check((bad / "corrupt-git").exists(), "dupes keeps a copy it couldn't read")
    check(not (july / "CoolProject").exists() and not (root / "stray-copy").exists(),
          "dupes still removes the copies that truly hold nothing new")
    check(all((good / n).is_dir() for n in ("cool-project", "fractal-lab", "synth-tools", "solver")),
          "dupes never touches the live mirror triage recommended")
