...
**Staring at a NAS full of duplicate copies instead?** Start with
`.\repo-sync.ps1 -Triage` — it only looks. See *The NAS has several complete copies of
everything*, below.
...
| `repo-sync.ps1` | maps `Z:` if needed, then starts the tool. Nothing else — all the work is in `repo_sync.py`, which is what the self-test covers |
...
## The NAS has several complete copies of everything

That's the old `repo_convergence.py` at work: it never found the repos already on the NAS, so
every run copied the lot in again. Nothing is lost, and nothing needs deleting today. Work
through it in this order — the first two steps change nothing at all.

```powershell
.\repo-sync.ps1 -Triage                       # 1. look. Read only
.\repo-sync.ps1 -Triage -Size -Log triage.txt #    ... with sizes, and a copy of the report
```

`-Triage` runs `repo_sync.py triage`, so it copes with what's actually on a share: repos with
...
worth keeping as the live mirror, and prints the next two commands with the paths filled in.

Old folder names don't matter. Repos are matched by **origin and history**, never by name, and
the sync renames NAS folders to match the P52. A tree full of `CamelCase_Names` from July is
still perfectly usable as the mirror.

Other places copies pile up — a second share, or the zip/archive folders on `D:` — get the same
treatment by pointing the triage script straight at them:

```powershell
py -3 repo_sync.py triage --root Y:\ --log triage-y.txt
py -3 repo_sync.py triage --root D:\gitzips
```

```powershell
.\repo-sync.ps1 -Dupes               # 2. list what holds nothing new. Deletes NOTHING
.\repo-sync.ps1 -Dupes -- --apply    # 3. delete those, after you type YES
.\repo-sync.ps1 -DryRun              # 4. then the normal plan
```

Step 3 has no zip-backup stage, unlike the sync: its safety is the proof in the next paragraph,
re-checked immediately before each folder goes. If you'd rather keep everything, stop after
step 2 and move the trees to one side yourself.

## What `-Dupes` will and won't delete

It searches the NAS share and `D:\code\.repo-convergence-archive` for repo copies. It only deletes a copy when:
...
Anything holding something unique is listed as kept, with the reason — a stray commit, an
uncommitted file, a `.env` that differs, a tag nothing else has, another working tree attached
to it, or simply that it couldn't be read. The live repos in `D:\code` and
the `--nas-base` folder are never touched by this, so point `--nas-base` at the tree you're
keeping before you run it. For other folders: `py -3 repo_sync.py dupes --root <folder> [--apply]`.
Copies more than three folders deep need `-Depth 5`.
...
Wrapper switches (these go *before* the `--`): `-DryRun`, `-Triage`, `-Dupes`, `-Only`, `-Size`
and `-Log <file>` (both for `-Triage`), `-Depth N` (for `-Triage` and `-Dupes`).

Three commands, all from the same script: `repo_sync.py sync` (the default), `repo_sync.py
dupes`, `repo_sync.py triage`. `--help` works on each.
...
- Windows 11 on the P52, Git for Windows 2.30+ on PATH, Python 3.10+ (`py -3`). PowerShell 5.1
  or 7.x both work; the wrapper only maps the drive and starts Python.
- `Z:` is a real mapped drive (`net use`), not a PowerShell-only `New-PSDrive` — Python can't
  see those. If a drive letter gives trouble, `--root \\192.168.1.50\GitHub-Repos` works too.
