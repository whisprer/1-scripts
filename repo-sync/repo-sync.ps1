...
Maps the NAS as Z: if it isn't already, then runs repo_sync.py:
    ...
    .\repo-sync.ps1 -Triage                  several copies on the NAS? start here. Read only
    .\repo-sync.ps1 -Triage -Size -Log t.txt  ... with sizes, and a copy of the report
    ...
    .\repo-sync.ps1 -Dupes -Depth 5          ... looking deeper than 3 folders down
    .\repo-sync.ps1 -- --allow-mass-delete   anything after -- goes straight to repo_sync.py

This wrapper only maps the drive and starts Python; all the work is in repo_sync.py,
which is the part covered by the self-test (py -3 test_repo_sync.py).
...
# NB: PowerShell variable names are CASE-INSENSITIVE, so no local below may be named after a
# parameter above - $triage would BE the -Triage switch and refuse to hold a path.
$here = Split-Path -Parent $MyInvocation.MyCommand.Path
$syncScript = Join-Path $here 'repo_sync.py'
...
# 3) Run
if ($Triage) {
    # read only: says which of the copies on the share to keep as the live mirror
    $argList = @($syncScript, 'triage', '--root', "$Drive\")
    if ($Size)       { $argList += '--size' }
    if ($Log -ne '') { $argList += @('--log', $Log) }
} elseif ($Dupes) {
    # looks for copies on the whole NAS share and in the old script's archive folder;
    # the live repos in D:\code and under --nas-base are never touched by this
    $argList = @($syncScript, 'dupes', '--root', "$Drive\")
    $oldArchive = 'D:\code\.repo-convergence-archive'
    if (Test-Path -LiteralPath $oldArchive) { $argList += @('--root', $oldArchive) }
} else {
    $argList = @($syncScript, 'sync')
    if ($DryRun) { $argList += '--dry-run' }
    foreach ($o in @($Only | Where-Object { $_ })) { $argList += @('--only', $o) }
}
if (($Triage -or $Dupes) -and $Depth -gt 0) { $argList += @('--depth', "$Depth") }
