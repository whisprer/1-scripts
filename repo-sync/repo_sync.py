...
DEFAULT_NAS_ROOT = "Z:\\"          # the whole share, for 'triage' and 'dupes'
...
def linked_worktrees(repo: Path) -> list[str]:
    """Other working trees checked out from this one. Deleting it would strand them."""
    try:
        return sorted(e.name for e in os.scandir(longpath(repo / ".git" / "worktrees")) if e.is_dir())
    except OSError:
        return []
...
def cmd_dupes(args: argparse.Namespace) -> int:
    ...
    for r in roots:
        if not os.path.isdir(longpath(r)):
            raise SystemExit(f"--root {r} not found" + drive_hint(r))
    ...
    for members in groups.values():
        ...
        for m in members:
            if m is keep or m.worktrees or m.path in deletable or _same_folder(m.path, keep.path):
                continue
...
# ---------------------------------------------------------------------------
# COMMAND: triage  (several complete copies on the NAS - which one do I keep?)
# ---------------------------------------------------------------------------

TRIAGE_SKIP_DIRS = CACHE_DIR_NAMES | {"$RECYCLE.BIN", "System Volume Information", "#recycle",
                                      "@eaDir", "@Recycle", ".stfolder", ".stversions", ".git"}
...
class TriageTree:
    ...
    direct: int = 0            # repos sitting directly inside (what --nas-base expects)
    kebab: int = 0
    in_p52: int = 0
    origins: int = 0
    sampled: int = 0
    ...
    @property
    def flat(self) -> bool:
        return bool(self.repos) and self.direct == len(self.repos)

    @property
    def usable(self) -> int:
        """Roughly how many of its repos could be reused as the mirror."""
        return 0 if not self.sampled else round(len(self.repos) * self.origins / self.sampled)


def is_repo_dir(p: Path) -> bool:
    return os.path.isdir(longpath(p / ".git"))


def drive_hint(p: Path) -> str:
    """A mapped drive can be missing for reasons that aren't obvious from 'not found'."""
    if not IS_WINDOWS:
        return ""
    drive = os.path.splitdrive(str(p))[0]
    if not re.fullmatch(r"[A-Za-z]:", drive or ""):
        return ""
    return (f"\n  {drive} isn't there for this process. Either:"
            f"\n    net use {drive} \\\\192.168.1.50\\GitHub-Repos /persistent:yes"
            f"\n  or skip the drive letter and use the share directly:"
            f"\n    --root \\\\192.168.1.50\\GitHub-Repos"
            f"\n  (a drive mapped in an elevated PowerShell isn't visible in a normal one, or the"
            f"\n   other way round, and a PowerShell-only New-PSDrive isn't visible to Python at all)")


def newest_commit(git: Git, repo: Path) -> int:
    """Newest commit across every ref, in unix seconds; 0 when there are no commits.
    A repo with no commits is normal here: 'git log -1 --all' prints NOTHING and exits 0."""
    r = git.run(repo, "log", "-1", "--all", "--format=%ct")
    text = r.out.strip()
...
def origin_url(git: Git, repo: Path) -> str:
    r = git.run(repo, "config", "--get", "remote.origin.url")
...
def cmd_triage(args: argparse.Namespace) -> int:
    ...
    def finish(code: int) -> int:
        if getattr(args, "log", ""):
            try:
                ...
                out(f"(report also saved to {args.log})")
            except OSError as e:
                out(f"(couldn't save the report to {args.log}: {e})")
        return code

    git = Git(find_git())
    root = Path(args.root)
    if not os.path.isdir(longpath(root)):
        raise SystemExit(f"--root {root} not found" + drive_hint(root))

    p52_base = Path(args.p52_base)
    p52_names: set[str] = set()
    try:
        for e in os.scandir(longpath(p52_base)):
            if e.is_dir(follow_symlinks=False) and is_repo_dir(p52_base / e.name):
                p52_names.add(e.name.lower())
    except OSError:
        pass

    say("")
    say(f"P52  {p52_base} : {len(p52_names)} repos")
    if not p52_names:
        say("  (none found there - is --p52-base right?)")
    say(f"NAS  {root} : looking for copies, up to {args.depth} folders deep ...")

    try:
        tops = sorted((root / e.name for e in os.scandir(longpath(root))
                       if e.is_dir(follow_symlinks=False)
                       and e.name not in TRIAGE_SKIP_DIRS
                       and TOMBSTONE_SUFFIX not in e.name),
                      key=lambda p: p.name.lower())
    except OSError as e:
        raise SystemExit(f"can't read {root}: {e}") from None

    trees: list[TriageTree] = []
    loose: list[Path] = []
    for d in tops:
        if is_repo_dir(d):
            loose.append(d)
            continue
        found = find_repos([d], max(1, args.depth - 1), set())
        if found:
            trees.append(TriageTree(d.name, d, found))
    if loose:
        trees.append(TriageTree("(loose at the top level)", root, loose))
    if not trees:
    ...
    for t in trees:
        n = len(t.repos)
        row = [t.name, str(n),
               "flat" if t.flat else f"nested ({t.direct}/{n} flat)",
               f"{t.in_p52} ({round(100 * t.in_p52 / n)}%)",
               f"{round(100 * t.kebab / n)}%",
    ...
    say("")
    say("What's on the NAS")
    say("  " + "  ".join(h.ljust(width[i]) for i, h in enumerate(head)).rstrip())
    say("  " + "  ".join("-" * width[i] for i in range(len(head))))
    for r in rows:
        say("  " + "  ".join(r[i].ljust(width[i]) for i in range(len(head))).rstrip())
    say("")
    say("  repos         repo folders found there")
    say("  layout        'flat' = the repos sit directly inside, which is what --nas-base expects")
    say(f"  also on P52   how many share a folder name with {p52_base} (name only; a rename reads as 'no')")
    say("  new names     folder names already in the all-lowercase-with-dashes style")
    say(f"  newest commit newest commit among the {args.sample} most recently touched repos there")
    say("  origins       how many of those had an 'origin' remote set")

    only_nas = sorted({p.name for t in trees for p in t.repos if p.name.lower() not in p52_names},
                      key=str.lower)
    say("")
    if not only_nas:
        say(f"Every repo name on the NAS also exists in {p52_base}. Nothing here is a lone copy.")
    else:
        say(f"{len(only_nas)} repo name(s) on the NAS with no same-named folder in {p52_base}:")
        for n in only_nas[:40]:
            say(f"    {n}")
        if len(only_nas) > 40:
            say(f"    ... and {len(only_nas) - 40} more")
        say("  Mostly old names, renamed on GitHub since. 'dupes' below checks by history, not by")
        say("  name, and keeps anything that really does hold a commit nothing else has.")

    flat = [t for t in trees if t.flat]
    best = flat[0] if flat else trees[0]
    me = "py -3 repo_sync.py" if IS_WINDOWS else f"python3 {Path(sys.argv[0]).name}"
    say("")
    say("Suggested plan")
    say(f"  Keep as the live mirror:  {best.path}   ({len(best.repos)} repos)")
    if not best.flat:
        say("  CAREFUL: every place here buckets its repos under a sub-folder, and --nas-base only")
        say("  looks at what's directly inside it. Point --nas-base at one of those sub-folders,")
        say("  or give it an empty folder and let the sync clone into it fresh.")
    if len(trees) > 1:
        say(f"  The other {len(trees) - 1} get checked for duplicates below.")
    say("  Old folder names don't matter: repos are matched by origin and history, and the sync")
    say("  renames NAS folders to match the P52. Nothing is identified by folder name.")
    say("")
    tail = f' --nas-base "{best.path}" --depth {args.depth}'
    say("  1) list what holds nothing new - deletes NOTHING:")
    say(f'       {me} dupes --root "{root}" --p52-base "{p52_base}"{tail}')
    say("  2) read that list, then delete (it asks you to type YES):")
    say(f'       {me} dupes --root "{root}" --p52-base "{p52_base}"{tail} --apply')
    say("  3) then bring GitHub, the P52 and that mirror into agreement:")
    say(f'       {me} sync --nas-base "{best.path}" --dry-run')
    say("")
    say("  Don't run the old converge.ps1 / repo_convergence.py again - that's what made the copies.")
    return finish(0)
...
def main(argv: Optional[list] = None) -> int:
    ...
    if not argv or argv[0] not in ("sync", "dupes", "triage", "-h", "--help", "--version"):
        argv.insert(0, "sync")
